      ...
      >
        <thead>
          <tr>
            ...
            <th>{{ 'ID' }}</th>
            <th>{{ 'Fullname in Khmer' | translate }}</th>
            <th>{{ 'Fullname in Latin' | translate }}</th>
        ...
        <tbody>
          <tr *ngFor="let data of smallTable.data; let i = index">
            ...
            <td>
              <div class="employee-profile">
                <img
                  [src]="data.imageUrl"
                  alt="Employee Photo"
                  *ngIf="data.imageUrl; else noImage"
                />
              </div>
              <ng-template #noImage>
                <div class="employee-profile">
            ...
            <td>{{ data.fullNameKh }}</td>
            <td>{{ data.fullName }}</td>
            <td>{{ data.sex }}</td>
            <td>{{ data.roleName}}</td>
            <td>{{ data.address }}</td>
            <td>
              <nz-upload nzAction="" [nzFileList]=""></nz-upload>
            </td>
            <td>
...
export class EmployeeListComponent implements OnInit, OnDestroy {
  constructor(
    protected uiService: EmployeeUiService,
  ...
  onSearch() {
    ...
    this.service.search(this.param).subscribe({
      next: (result: { results: Employee[]; param: QueryParam }) => {
