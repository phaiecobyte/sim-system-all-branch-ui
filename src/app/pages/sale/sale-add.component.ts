...
export class SaleAddComponent  implements OnInit, OnDestroy {
  ...
  onSubmit(): void {
    if (this.frm.valid) {
      ...
      // Filter out any empty order details
      const validOrderDetails = formData.orderDetails.filter(
        (detail: any) => detail.productID && detail.qty && detail.price
      );

      // Format the request object according to API requirements
      const orderRequest: any[] = [{
        ...
        refer: Number(formData.refer || 0),
        credit: '',
        orderDetails: validOrderDetails.map((detail: any) => ({
          id: 0,
          orderID: 0,
          productID: detail.productID,
          colorCode: '',
          unitTypeID: detail.unitTypeID || 0,
          productUnitId: detail.productUnitId || 0,
      ...
      // Only proceed if there are valid order details
      if (orderRequest[0].orderDetails.length === 0) {
        this.notification.showError('Please add at least one product');
        this.loading = false;
        return;
      }

      this.orderService.add(orderRequest[0]).pipe().subscribe({
        next: async () => {
          this.loading = false;
        ...
        error: (err: any) => {
          ...
          if (err.error && err.error.message) {
            ...
            this.notification.showError('Failed to save order');
          }
        }
      });
    }
